...
# main one is a route row; each sub-command has the shape 4.4 promises (a name matching its
# directory, user-only invocation, a body that loads the main skill, and, on every route but
# the update log, the mode token); and every section a route row cites is a file.
# Run: bash tests/sub-commands.test.sh
...
shape_defects() {
  ...
  if [ "$1" = log ]; then
    [ -z "$(front_value "$2" arguments)" ] || printf '%s: takes arguments\n' "$1"
  else
    [ "$(front_value "$2" arguments)" = '[mode]' ] || printf '%s: arguments is not [mode]\n' "$1"
    grep -q -F "\$mode" "$2" || printf '%s: never reads the mode token\n' "$1"
    grep -q -F 'agentless' "$2" || printf '%s: never names agentless\n' "$1"
  fi
  lines=$(wc -l < "$2" | tr -d ' ')
...
# stray_dirs <skills dir>: every skill directory that is not a table route.
stray_dirs() { comm -23 <(skill_dirs "$1") <(table_routes "$1/$MAIN_SKILL/SKILL.md" | sort); }
...
# sub-command missing its mode token and a cited section with no file are all named.
test_planted_breaches_are_named() {
  mkdir -p "$WORK/skills/$MAIN_SKILL" "$WORK/skills/ghost" "$WORK/skills/quick"
  ...
  assert_contains "a sub-command with no mode token is named" "quick: arguments is not [mode]" "$(all_defects "$WORK/skills")"
  tick=$(printf '\140')
