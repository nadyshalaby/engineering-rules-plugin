...
description: The agentless sub-command with a request keeps every helper shape in the main conversation, so the Agent tool is never called, while the ledger, the scouts and the review still run.
...
---
/engineering-rules:agentless add a function `sum(a, b)` that returns `a + b` to a new file `sum.js` in this directory, exported as a CommonJS module, with a test in `sum.test.js` that runs under `node --test`
