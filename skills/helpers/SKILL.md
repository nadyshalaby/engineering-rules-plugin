---
name: helpers
description: "Helpers on for the engineering-rules law: agentless is the default, and this switch sends the helper shapes (readers, the stage-end scouts, the fresh-eyes reviewers, a mechanic, a builder) through the agent tool, for the task in flight and every task after it in this conversation, until you say off. Usage: /engineering-rules:helpers [what you want done | off]"
argument-hint: "[what you want done | off]"
disable-model-invocation: true
---

# Helpers on, by the user's word

The user typed `/engineering-rules:helpers`. Agentless is the default helper mode (4.4, 1.8):
every helper shape (5.5) runs in this session and nothing is sent to the agent tool. This
switch turns helpers on, for the task in flight and for every task that starts in this
conversation, until they switch back. It picks no route; the routes have sub-commands of
their own, and this one combines with any of them.
...
2. **Read the invocation.**
   ...
   - Empty, with no task in flight: say in one line that helpers are on for this
   ...
   - Anything else is the request. Helpers are on for it, and it classifies under 4.1
     exactly as prose typed into `/engineering-rules:engineering-rules` does: full mode
     unless the words name another route.
3. **With a request, say the route and the mode in one line** (1.7, step 2), *"Helpers on,
...
Everything after that is the law, unchanged. Helpers on moves the reading out and removes
