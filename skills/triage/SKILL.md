---
name: triage
description: "Review triage under the engineering-rules law, picked by your word: a batch of review findings becomes a per-finding accept, push-back, defer or needs-restatement table. Usage: /engineering-rules:triage [agentless] <the pasted findings>"
argument-hint: "[agentless] [the pasted findings]"
arguments: [mode]
...
3. **Read the findings.** They are the whole invocation with the mode token removed or, when
   nothing is left, the batch pasted most recently in this conversation; when there is
   neither, ask for them through the wizard tool (4.4), one question, and nothing else.
4. **Say the route and the mode in one line** (1.7, step 2): *"Review triage."*, then
   *"Agentless."* with the cost line 4.4 names when it is on.
5. **Read 16.2 from disk and run it.** The always-on law binds throughout; a finding accepted
