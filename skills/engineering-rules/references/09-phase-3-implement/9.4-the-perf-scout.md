...
Run it BEFORE reading any candidate, over the same path list the run point handed you. That list is NUL-separated (9.5, step 1), so hand it over through `scout_coverage_from_list "$SCOUT_PATHS"`.
...
# scout_coverage_from_list <file>: the NUL-separated list 9.5 builds, handed over as arguments.
scout_coverage_from_list() {
  cov_list=$1; set --
  while IFS= read -r -d '' cov_p; do set -- "$@" "$cov_p"; done < "$cov_list"
  scout_coverage "$@"
}
```
