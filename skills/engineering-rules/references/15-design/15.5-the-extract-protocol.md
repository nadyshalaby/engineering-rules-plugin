# 15.5 [protocol] The extract protocol
...
Output always conforms to 15.3. Direction naming comes from 15.4.
...
| An image-generation tool exists and the task is visually led | **C, from image**, with the images generated first under 15.31 |
| A `DESIGN.md` exists and the product has drifted from it | **REFRESH** |
| Nothing exists yet | Not an extraction. Author from 15.4 via 15.19. |
...
- **Do not ship a proprietary font as if it were available.** When the reference uses a licensed face, record it as `name` and supply a freely-licensed `substitute` with tuning notes (15.3, fonts).
...
1. **Find the token source.** Search in this order and stop at the first that exists: a utility-framework config, a stylesheet entry point with custom properties, a `theme` or `tokens` module, a native theme file (`Theme.of`, `ColorScheme`, asset catalog), then component-level styles. Record which file you took each block from. When the token source is an official design system's package (15.21), record the system in the spec's `system` block and map the roles to its token names; do not harvest its values as if they were the product's own.
...
8. **Name the direction.** Match the recovered system against 15.4. If two directions fit equally, the product is mixing directions, say so plainly in the report, because that is a finding.
...
**Generated images.** When the images came from the image-first workflow (15.31), the scale anchor is the frame width the tool rendered at, the deep-analysis checklist in 15.31 is the extraction procedure for each section image, and a section too small to read is regenerated, never cropped or zoomed. The extraction report still states a confidence per block.
