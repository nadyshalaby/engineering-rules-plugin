# 15.14 [direction] Direction, data-dense
...
**Palette logic.** Deep neutral field with two or three surface values for row banding and review separation. Color is reserved almost entirely for **data encoding**: positive, negative, and a small categorical set. Chrome stays achromatic so the data is the only colored thing on screen.
...
**Dials.** `DESIGN_VARIANCE` 2, `MOTION_INTENSITY` 1, `VISUAL_DENSITY` 9 (15.20). The top of the density band; the marketing composition law (15.22) does not apply to these surfaces.
