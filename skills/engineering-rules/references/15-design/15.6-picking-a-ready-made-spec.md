# 15.6 [reference] Picking a ready-made spec

## Catalog (twelve ready-to-drop design specs)

Twelve complete `DESIGN.md` files, one per direction in 15.4, each conforming to 15.3.
...
|---|---|---|---|
| industrial-precision | dark | engineered, exact, unsentimental | Developer tools, infrastructure dashboards, trading and ops consoles, B2B admin |
| editorial-print | light | authored, considered, worth reading | Publications, long-form docs, research sites, idea-led marketing |
| retro-terminal | dark | direct, expert, no hand-holding | CLIs and their web companions, self-hosted tooling, security products |
| warm-organic | light | human, unhurried, safe | Health and wellness, community and education, finance for non-experts |
| brutalist-mono | light | honest, loud, uninterested in charming you | Portfolios, agencies, launch and event sites, music and culture |
| neo-luxury | dark | scarce, slow, expensive | Premium commerce, hospitality, private banking, membership products |
| swiss-grid | light | rational, ordered, neutral | Institutions, universities, design-system docs, professional services |
| data-dense | dark | professional, information-first, earned | Analytics, trading and risk, observability, logistics, power-user admin |
| playful-pop | light | energetic, tactile, low-stakes | Consumer mobile, kids and education, games and community, habit tracking |
| nordic-calm | light | quiet, focused, unbothered | Writing and notes, focus and productivity, reading apps, meditation |
| cyber-neon | dark | charged, kinetic, after-hours | Gaming and esports, streaming and creator tools, crypto, security |
| soft-depth | light | modern, layered, approachable | General-purpose product UI, consumer SaaS, cross-platform apps |
...
2. **Copy** it to `<project>/docs/design/DESIGN.md`. The section's fenced `yaml`
   block becomes that file's frontmatter, verbatim; the prose below it follows. Promote
   every heading one level as you copy (`###` becomes `##`, `####` becomes `###`); the
   headings are demoted here only because the spec sits inside a reference file. The copied
   file must carry the eleven `##` sections 15.3 lists, in that order.
...
4. **Update the header**: set `name` to the product, keep `direction`, rewrite `description` for the product, and record `dials` when the read (15.20) moved them off the profile's default.
5. **Recompute the contrast ratios** after any palette change and update the Colors section. The stated ratios must be true.
6. **Generate the preview** and commit it beside the spec: one self-contained page that
   renders every token and every component from the spec, keeps component entries as
   their `{token.ref}` strings so an unresolved reference shows up visibly, and carries
   zero network references.
7. **Validate** against the checklist at the end of 15.3.

The design-spec route (15.19) runs this whole sequence, including the extraction path when the project already has tokens.
