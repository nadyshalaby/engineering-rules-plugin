# 15.10 [direction] Direction, warm-organic
...
**Dials.** `DESIGN_VARIANCE` 5, `MOTION_INTENSITY` 5, `VISUAL_DENSITY` 3 (15.20). Motion at 5 means settling, not springing.
...
**Anti-tells.** Pastel purple and pink (the canonical AI-design cliché). Uniform radius everywhere. Cold greys mixed into a warm palette. Bouncy spring motion, which belongs to `playful-pop`. Illustration styles that fight the palette. `Fraunces` reached for as a mood on a product that is not on this direction, which is the serif reflex (`tell.type.serif-reflex`, 15.24); here the face is named on purpose, and the rotation rule (15.20) still asks for a different display face on the next consecutive project. The beige-brass-espresso palette worn by every generated craft brand (`tell.visual.premium-beige-brass`): sand and clay are this direction, but the accent is terracotta or moss, never brass, and the text is warm near-black, never espresso.
