# 15.3 [contract] The design spec contract

## DESIGN.md (Spec Contract)

The binding anatomy of a design spec under this law. A `DESIGN.md` is the **committed visual contract** for a product: machine-readable tokens a coding agent can resolve, and prose a human can argue with. Code is written against it and reviewed against it.

Read this file when authoring, refreshing, extracting, or auditing a spec. Governed by the visual law (15.1); direction choice comes from the direction library (15.4).
...
**Never** author a second spec beside an existing one. Refresh the existing file (15.5, REFRESH mode).
...
direction: <slug from the direction library>
...
description: >
  ...
  it recognizable. Written so someone reading ONLY this paragraph would
  build something close.
```

`direction` MUST be one of the twelve slugs in 15.4, or `custom` with a `direction_rationale` key explaining why none fit.
...
**Optional: `dials`.** The three dials from the read (15.20), recorded so the next session inherits the decision rather than the baseline:

```yaml
dials: { variance: 6, motion: 3, density: 3 }   # DESIGN_VARIANCE, MOTION_INTENSITY, VISUAL_DENSITY, each 1-10
```

Omitted, the direction profile's default triple (15.7 to 15.18) applies. The Overview prose states the same numbers with one clause of reasoning each.

**Optional: `system`.** Present when the project sits on an official design system (15.21). The spec then documents how the product uses the system, and every role below maps to a system token name rather than inventing a value:
...
system:
  ...
  version: "2.x"
  tokens: "md-sys-*"              # the token namespace every role maps to
```

With `system` set, `direction` names the nearest slug so the profile's anti-tells apply, and validation item 13 checks the mapping.
...
- **`substitute` is a freely-licensed family** an implementation can install today. When `name` is proprietary, `substitute` MUST differ from it and the Typography prose MUST say how the substitute is tuned to approximate the original (weight, tracking, feature settings).
- **Never** cite a webfont URL anywhere in the spec or the preview.
- The banned-by-default families of 15.1 stay banned as the *display* face.
- **The display face rotates** between consecutive projects (15.20, the rotation rule). The same face on two specs in a row is a finding unless the second inherits an official system.
...
One token set, four renderers. This table is normative: an implementation applying the spec on any listed platform reads the same row.
...
| # | Section | Job |
|---|---|---|
| 1 | `## Overview` | Opens with the one-line read and the three dials (15.20), and the `Rotated away from` line when a previous project exists. Then two to four paragraphs on the atmosphere, plus a `**Signature moves:**` list of 4-7 bullets naming what makes this system recognizable at a glance. |
...
| 7 | `## Motion` | Duration and easing scale, what animates and what must not, the one orchestrated moment per screen, the reduced-motion behavior, and the one-sentence reason for every class of animation the product carries (15.25). |
...
| 9 | `## Platform & Responsive` | Breakpoint table with key changes, touch targets, the under-768 collapse of every multi-column layout (15.22), native idioms, and RTL behavior via logical properties. |
...
| 11 | `## Prompt Guide` | Copy-paste prompts an implementation can hand to a coding agent, plus the token quick-reference. Ends with the three rules that matter most if everything else is forgotten. |

---

### Authoring rules

1. **Size.** 380-470 lines. The complete token set costs roughly 240 lines before any prose (twelve typography roles, twenty component entries with states, the platform block), so a spec much under 380 is missing required tokens. The hard cap on any one file is 500 (1.1); a spec pushing past 470 is documenting screens instead of a system.
...
7. **Direction discipline.** One direction, executed cleanly. A spec that mixes two directions has no point of view (15.1).
8. **Dark and light.** State which mode is canonical. If both ship, give the inverted values for every color role, not a vague "invert the palette", and the second mode keeps the hierarchy and the brand of the first: the accent stays recognisable, elevation still reads, contrast is recomputed per mode, and the preview's toggle proves it (15.26, the dark-mode protocol).
...
13. When `system` is set, does every `colors` and `typography` role name the system token it maps to, and does `direction` name the nearest slug? (yes / no, or n/a with no `system` block)
