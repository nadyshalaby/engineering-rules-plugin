# 15.11 [direction] Direction, brutalist-mono

## Brutalist Mono (`brutalist-mono`)
...
**Dials.** `DESIGN_VARIANCE` 9, `MOTION_INTENSITY` 4, `VISUAL_DENSITY` 4 (15.20). The one direction where variance sits in the top band by default.
...
**Anti-tells.** Brutalism as an excuse for illegible contrast or broken accessibility. Random rotation applied to everything. Comic Sans irony. Any shadow at all. Cluttered "chaos" with no underlying grid, since real brutalism is rigorously gridded. Two substrates on one page: pick one (paper, concrete, screen) and hold it, because concrete texture beside a glossy card is a costume, not a direction.
