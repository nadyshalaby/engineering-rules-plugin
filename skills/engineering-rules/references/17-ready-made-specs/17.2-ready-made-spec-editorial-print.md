# 17.2 [spec] Ready-made spec, editorial-print
...
Color is scarce on purpose. Oxblood `{colors.accent}` appears on links, on rules that mark a section boundary, and on the single primary action. Everything else is ink, paper, and the greys between them. When an editorial page needs visual weight it reaches for a photograph or a headline, never for a colored review.
...
- Lead a page with a photograph or a headline, never with a colored review.
