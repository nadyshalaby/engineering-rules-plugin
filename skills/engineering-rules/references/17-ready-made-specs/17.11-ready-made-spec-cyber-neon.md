# 17.11 [spec] Ready-made spec, cyber-neon
...
That restriction is what separates this direction from the purple-to-pink gradient wash that has become the default AI aesthetic. This direction sits closer to that cliché than any other in the catalog, and the discipline that keeps it clear is simple: **color is emitted by small things**. A screen with a large cyan review has already failed.
...
- Wide-tracked mono overlines at 0.18em labelling every review.
...
| `{typography.heading-lg}` | 27px | 600 | 1.2 | -0.2px | review title |
...
| `{typography.overline}` | 11px | 500 | 1.2 | 0.18em | Mono uppercase review labels |
...
| Level | Treatment | Use |
|---|---|---|
| 0 | Flat with a `{colors.hairline-strong}` border | Every review and card at rest |
...
| `{motion.duration.base}` | 240ms | review and dropdown entry |
...
3. **The load sweep.** On first paint, a narrow cyan highlight sweeps once across the top edge of the primary review over 800ms, then stops. Once per view.
...
|---|---|---|
| `xl` | ≥ 1440px | Full multi-review layout; grid overlay visible; corner cuts at 14px |
| `lg` | 1024-1439px | Panels reduce to two columns; cuts at 12px |
| `md` | 768-1023px | Single review column; grid overlay removed |
