# 17.1 [spec] Ready-made spec, industrial-precision
...
| `{typography.heading-lg}` | 26px | 600 | 1.2 | -0.3px | review title |
...
**Section rhythm:** `{spacing.huge}` between major sections on content pages, `{spacing.xxl}` between panels on dense surfaces, `{spacing.lg}` between rows within a review. Whitespace is generous between groups and tight within them, so grouping is legible before anything is read.
...
| `{motion.duration.base}` | 240ms | review and dropdown entry, tab switches |
