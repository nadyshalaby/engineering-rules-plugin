# 17.8 [spec] Ready-made spec, data-dense
...
The governing rule is that **color belongs to the data, not to the chrome**. Navigation, review headers, borders, and labels are all achromatic: `{colors.text-secondary}` and `{colors.hairline}` and nothing else. Color appears when a value is positive or negative, when a series needs distinguishing in a chart, or when a row is selected. The moment chrome takes a color, the data stops being the only colored thing on screen and scanning gets slower.
...
#### Surface
- **Canvas** (`{colors.canvas}`, `#0d1013`): the outermost field and the gutter between review groups.
...
- **Hairline Strong** (`{colors.hairline-strong}`, `#2d3841`): review edges, header rules, input borders.
...
| `{typography.display-lg}` | 24px | 600 | 1.2 | -0.4px | review headline figure |
| `{typography.heading-lg}` | 18px | 600 | 1.25 | -0.2px | Page title |
| `{typography.heading-md}` | 15px | 600 | 1.3 | 0 | review title |
| `{typography.heading-sm}` | 13px | 600 | 1.3 | 0 | Sub-review title |
...
| Level | Treatment | Use |
|---|---|---|
| 0 | Flat, hairline border | Every review, card, table |
...
| Token | Value | Use |
|---|---|---|
| `{rounded.none}` | 0 | Table cells, review edges, chart areas |
...
| `{motion.duration.base}` | 120ms | Dropdown open, review resize |
...
| `lg` | 1024-1439px | Side review collapses to icons; low-priority columns hidden |
| `md` | 768-1023px | Single review column; table scrolls horizontally with a frozen key column |
...
- Don't color the navigation, review headers, or borders.
