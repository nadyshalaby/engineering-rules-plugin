# 17.12 [spec] Ready-made spec, soft-depth
...
#### Semantic
**Positive** (`{colors.positive}`, `#157a4c`, 5.00:1), **Caution** (`{colors.caution}`, `#8a5f10`, 5.26:1), **Negative** (`{colors.negative}`, `#bf3225`, 5.30:1). Each follows the accent's pattern: a solid color for text and icons, an 8 to 12 percent tint for banner and badge backgrounds. Never a fully saturated background review.
...
| `{typography.heading-md}` | 20px | 600 | 1.35 | -0.2px | Card and review title |
...
**Three layers maximum.** Canvas, surface, raised. A fourth layer (a card inside a card inside a review) makes the hierarchy unreadable, and the correct fix is to flatten the nesting rather than to invent another value.
