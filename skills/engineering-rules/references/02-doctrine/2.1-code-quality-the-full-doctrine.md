...
- A change deletes what it orphans, in the same stage: when the last caller, import or reference of a symbol, file, type, constant, fixture, flag, config key, env var or translation goes, the thing goes with it (9.1, the stage-end sweep).
- Dead code already in a touched file is removed in its own cleanup commit with the zero-reference proof per symbol (13.2, class 10); what that class lists as looks-dead-but-may-not-be is offered instead of deleted.
...
- The Boy Scout rule as a licence: pre-existing smells are follow-ups, never edits in this diff (3.4, "Deliberately not adopted"). Dead code in a touched file is a leftover, not a smell: it goes, in its own cleanup commit (13.2, class 10)
