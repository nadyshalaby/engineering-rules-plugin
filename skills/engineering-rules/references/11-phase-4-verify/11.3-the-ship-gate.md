...
The ship gate is the last mechanical proof in Phase 4: run the thing, watch it come up, drive the main flow. It is **always-on in every mode** (full mode, quick) and takes no user opt-in.
...
- Always tear down: kill the process group, bring compose down, free the port. A leaked server is a Phase 6 Step D, class 11 cleanup finding (13.2).
...
- **Migrations count as part of boot.** If the app needs a schema, run the project's migrate command first and treat its failure as a boot red. A migration that only works on a fresh database is a Critical finding (`sec.migrations.non-idempotent`, 3.2).
...
| "The build warning is pre-existing, ignore it" | Capture it, then check it against the base commit. Pre-existing goes to Phase 6 Step D, class 10 (13.2); new is a Phase 5 finding. |
...
- 11.2, the Evidence Ledger these rows live in, and the three-layer re-verify that runs beside them.
- 5.1, the exit artifact that makes Phase 4 un-tickable without the three ship rows.
- 13.2, Phase 6 Step D, where a leaked process or a pre-existing build warning lands.
- 6.13, the repo brief that carries the build, start and test commands these legs reuse.
