# 16.2 [route] Review triage

## Finding triage (structured response to reviewer findings)

This route converts a batch of review findings into a per-finding response table. Every finding becomes one row. Every row carries a Decision (`accept` / `push-back` / `defer` / `needs-restatement`) and Evidence anchored in code, not opinion. It is the detail behind the merged table in 12.9.

### When to invoke

Two trigger paths, both end in the same output table.

**Path A. Phase 5 internal.** Runs in every mode once the five checks (12.3 to 12.7) have filed and the challenge (12.8) has ruled. Before you decide what to fix, what to push back on and what to defer, this route writes the per-finding Decision and Evidence that 12.9's merged table carries. The Critical / Important / Minor tags from the checks are mirrored into the Severity column verbatim.
...
**Triggers (4.1).** This route is picked by its sub-command, `/engineering-rules:triage` (4.4), or when the user says "respond to the review", "respond to PR feedback", "respond to reviewer comments", "address review findings", or pastes review-shaped feedback and asks for a response.

**The one carve-out.** A single external comment with one obvious fix may be handled inline, with its Decision and Evidence stated in one line in chat. The table is mandatory for 2+ findings, and always on Path A.
...
Performance severities follow the severity model in 3.1. The classes named above are anchors, not the full enumeration; a finding that cites a `perf.<domain>.<slug>` ID takes that ID's catalog default, moved at most one level by context.
...
Findings batch (from a Phase 5 review of a small auth-token refactor, each tagged with the check that filed it):

1. the security check (12.3): `src/auth/token.service.ts:67`, token expiry check uses `Date.now()` directly; should inject a clock for testability.
2. the security check: `src/auth/token.service.ts:142`, refresh-token rotation path does not invalidate the old token before issuing the new one. Critical.
3. the quality-and-plan check (12.7): `src/auth/types.ts:8`, inline object type `{ userId: string; exp: number }` on the `TokenPayload` field; extract to a named interface.
4. the quality-and-plan check: `src/auth/token.service.ts`, file is 480 LOC, close to the 500-LOC cap; consider splitting now.
5. the quality-and-plan check: work-doc acceptance bullet D3 (CSRF cookie rotation) has no corresponding diff hunk.
6. the performance check (12.4): `src/auth/token.service.ts:118`, refresh-history lookup loads every row for the user, then filters in app code (perf.data.fetch-then-filter). Important.
...
| Inline object type `{ userId; exp }` on `TokenPayload` at `types.ts:8`; extract to named interface | Important | accept | `refactor: extract TokenPayload to named interface in auth/types/auth.types.ts`, per the no-inline-types rule in 2.1. |
...
The five checks are 12.3 to 12.7, the challenge is 12.8, the merged table and the escalation are 12.9.
