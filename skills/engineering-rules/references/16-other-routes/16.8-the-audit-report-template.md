# 16.8 [template] The audit report template

## Codebase audit (compliance report)
...
{{One opinionated recommendation, e.g. "Fix the 3 critical secrets first, then the 12 high
bans in one batch; defer the low naming findings." Then ask, through the wizard tool, whether
to proceed to remediation (Phase 5), and whether to fix top-down by severity or by file.}}
