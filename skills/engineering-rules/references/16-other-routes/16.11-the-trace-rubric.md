...
The viewer renders whatever the trace produces. A polished viewer over a sloppy trace is worthless; an ugly viewer over a sharp trace still ships decisions. This rubric is the substance of the route.

Walk depth-first from the entry point. Capture exactly the fields named in 16.10. When in doubt at a fork, **stop and ask**, never guess which branch the runtime takes.
...
If the repo has a `README.md`, `CONTRIBUTING.md`, `ARCHITECTURE.md`, or `docs/` at the root, skim them. If it has a `CLAUDE.md` or `.cursorrules`, read it; the user wrote those for tools like you.

---

### 1. Confirm the entry point. Do not guess.

The user's request names ONE entry. Resolve it to a single function with a file path and a line range. If any of the following are true, **stop and ask the user**:
...
| Tenant-guarded entry | Same handler delegated by tenant ID | "This handler run by `req.tenant.id`. Which tenant's path?" |
...
| Dynamic run | `handlers[type](payload)` | "`handlers[type]` is a map. Which `type` value should I trace?" |
...
For every function on the path, extract the fields from 16.10 in this exact order:
...
3. **Identify the invoked block.** See section 3 below, this is the hardest part.
...
8. **`docblock.side_effects`**, pick from the fixed set: `db`, `queue`, `http`, `cache`, `auth`, `fs`. See section 4 below for what falls into each.
...
10. **`data_in` / `data_out`**, the payload as it enters / leaves. See section 5 below, the trace's job is to show how shape transforms across the path.
11. **`risk`**, exactly ONE concrete risk, smell, or load-bearing assumption. Not three. Pick the one that matters most. See section 6 below.
12. **`branches_not_taken`**, every `if`/`else`/`switch`/`try`/`catch` arm in this function that the runtime DID NOT take on this path. Name + one-line trigger condition. **Do NOT expand them into nodes.** See section 7 below.
...
If the user later wants to trace the deferred branch, that is a new walkthrough run with a different entry-point hint (e.g., "the admin override path of `getUser`"). New `data.json`, new viewer.
...
Ask through the wizard tool (options: continue, switch branch, stop) and wait for the user before continuing. The defaults are:
