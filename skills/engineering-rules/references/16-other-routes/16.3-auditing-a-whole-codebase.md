...
Picked by its sub-command, `/engineering-rules:audit [a project root or directory]` (4.4), or
by the route table when the user asks to audit the codebase or the repo (4.1).
...
- **Ban tokens**, the bans in 1.1 (the full token list is the law scout's, 9.5, and the
  per-language analogs are 16.7's) plus the project's ban-patterns file verbatim if it has
...
(suppressions including the coverage-ignore markers, empty catch and its promise form, bare
Error with or without `new` and thrown literals, non-null `!`, inline type in scoped modules,
focused and unticketed skipped tests, debug artifacts, hardcoded secrets). Secrets are
redacted in the snippet. The scanner already applies the path carve-outs. Most findings are
`confidence: exact`; the three marked `confidence: syntactic` (`ban.bare-error`,
`ban.inline-type`, the `console.*` half of `clean.debug-artifact`) are matched exactly but need
a one-step check before you act, is the `throw` in domain code, does the type have 2+ props,
is the `console.*` in domain code.
...
- **Clean code**, the judged rows of the clean-code catalog (3.4): naming, function shape,
  comments, object design, error handling, boundaries, async ownership and test hygiene, each
  finding citing its `style.<domain>.<slug>` and honoring the "deliberately not adopted" table.
- **Cross-file cleanup**, unused dependencies, unreferenced files, dead feature flags, orphaned
  env vars, commented-out code blocks (the deterministic scanner already flags `// removed:`,
  ownerless TODO/FIXME and debug artifacts).
