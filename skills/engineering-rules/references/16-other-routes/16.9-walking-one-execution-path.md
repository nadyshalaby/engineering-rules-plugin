# 16.9 [route] Walking one execution path

## Code walkthrough (interactive call-stack viewer for code you didn't write)
...
> 5. **Light mode is default.** The viewer boots in light mode. `?theme=dark` URL param or the header ☾ toggle switches to dark; `localStorage["walkthrough-theme"]` persists the choice. Do not override the default in chat narration.
> 6. **Stop on ambiguity. Never guess.** Multiple matching definitions, runtime-conditional registration, tenant guards, DI-token resolution, dynamic dispatch. STOP and ask which path the user wants, through the wizard tool. See 16.11, section 1.
>
...
Two reference files carry the heavy detail:

- 16.10, the exact JSON contract the viewer consumes.
- 16.11, how to walk the stack with rigor (invoked-block detection, side-effect classification, picking the one risk, deferred-branch enumeration, depth check).

Read both before Phase 3 of this route on first invocation in a session. The always-on law binds throughout: nothing in the traced repo is edited, every question goes through the wizard tool, every claim in a docblock is derived from the code.

### Depth & Completeness Mandate (read once per session, non-negotiable)

**A code walkthrough is deep-by-default.** When the user asks for a trace, they are paying for the full call stack. The viewer's value collapses if the trace stops at the controller boundary and gestures at "calls `service.foo`" without recursing into `foo` and its own callees.
...
| **No surface-only summaries.** | A trace whose `nodes.length === 1` for a non-trivial endpoint is a bug, not a feature. If the trace pass produces only the controller method, redo it. |
...
The 5-function depth-check pause from 16.11 section 8 is for **interactive single-entry mode only**. In playbook mode, batch mode, or any invocation that includes the phrase "all", "every", "deep", "end to end", "full", or "complete", the depth check is **disabled**, the pass runs to leaves without pausing. The user has already pre-approved depth by asking for completeness.
...
This route is picked by its sub-command, `/engineering-rules:walkthrough <entry point>` (4.4), or (4.1) when the user names an entry point and wants to see how it works end to end.
...
**Do not invoke** for one-shot questions answered by a single grep ("where is X defined", "find usages of Y"). Those are a read-only ask. This route produces a viewer artifact; that is overkill for a definition lookup.
...
#### Phase 1 (Confirm the entry point. Ambiguity stops the walkthrough.)

Parse the user's request for a single entry. Resolve it to one file + line. If ANY of the conditions in 16.11 section 1 apply (multiple matching definitions, runtime-conditional registration, tenant guards, DI token resolution, dynamic dispatch), STOP and ask the user which path to trace, through the wizard tool. Never guess.
...
**Depth check is OFF when the user pre-approved depth.** In interactive single-entry mode, the 5-function depth check from 16.11 section 8 still fires. In playbook mode, batch mode, or any invocation that includes "all", "every", "deep", "end to end", "full", or "complete", the pass walks to leaves without pausing.
...
**Step 4.0 (mandatory). Check for an existing trace at `.walkthrough/<slug>/data.json`.**

```bash
test -f .walkthrough/<slug>/data.json && echo EXISTS || echo NEW
```
...
If `NEW`: fresh trace, `previous_generated_at: null`, `diff_vs_previous: null`. Print one line to chat: `creating new trace at .walkthrough/<slug>/data.json`.

If `EXISTS`: print `updating existing trace at .walkthrough/<slug>/data.json (last run: <previous_generated_at>)` plus a one-line diff summary (e.g. `+3 nodes, -1 removed, 2 signature_drift, 1 new side-effect`).

**Step 4.1. Build the full JSON.** Match 16.10 exactly. Field names are strict. Validate before writing:
...
Create `.walkthrough/<slug>/` at the repo root and write four files into it, then
`data.json` beside them:

| File | What it has to do |
|---|---|
| `index.html` | the page shell, loading the three files below |
| `viewer.js` | reads `data.json` and renders the call stack, the node detail pane and the sequence diagram |
| `viewer.css` | the styling, light mode by default |
| `serve.js` | a small static server over this folder, no dependencies |

All four are **self-contained**: zero network references, no CDN, no build step. `data.json` is the only file that varies between traces, and its shape is the trace data schema (16.10). Reuse the four from an existing `.walkthrough/<other-slug>/` when the repo already has a trace; write them fresh when it does not.

Verify all four files exist in `.walkthrough/<slug>/` before you finish. If any is missing, stop and report: the viewer is unusable without all four.

**`.gitignore`.** If `.gitignore` exists at the repo root and does NOT already contain `.walkthrough/`, append the line. If `.gitignore` does not exist, print this one-liner for the user:

```
echo '.walkthrough/' >> .gitignore
```

`.walkthrough/` is an ephemeral artifact, it doesn't belong in commits.

#### Phase 6 (Launch.)

Run from inside `.walkthrough/<slug>/`:

```bash
cd .walkthrough/<slug> && node serve.js
```
...
**Five comprehension questions** answerable only by someone who internalized the flow. Examples and intent are in 16.11 section 9. Each question targets a specific layer boundary, side-effect, branch-not-taken, or data-shape transition observed in this trace.
...
The default for a re-run on the same entry point is **update**, not regenerate. Phase 4 Step 4.0 above documents the merge-and-diff contract. Only when the user explicitly types "regenerate" or "fresh" does the trace overwrite without preserving manual edits or computing `diff_vs_previous`.

### Playbook mode, multi-entry code walkthroughs
...
In playbook mode the deliverable is the same per-slug viewer, *plus* a top-level `.walkthrough/index.html` that lists every traced entry, grouped by domain, with a live filter and method-color chips. Each row links into its sibling slug folder's viewer in a new tab.
...
Phase 5' (write assets + run _build.mjs) → Phase 6' (launch) →
...
**Phase 2', light or dark.** Ask the user once, through the wizard tool: light (default, matches single-entry mode and the locked contract) or dark. The viewer ships both and honors the `?theme=` URL param plus a `localStorage` preference; the build itself takes no flag, the viewer toggles at runtime.

**Phase 3', author `_catalog.json`** at `.walkthrough/_catalog.json`. Schema is in 16.10, "Playbook mode". Required fields per entry: `slug`, `method`, `route`, `domain`, `summary`, `controller` (or `entry`). The `slug` derives by the same rules as single-entry mode.

**Phase 4', optional `_traces.json`** at `.walkthrough/_traces.json`. When present, each entry's rich nodes/edges populate that slug's `data.json` directly. When absent, the builder writes a stub per slug, the user can deepen any specific slug later by walking that entry point in full.

**Phase 5', materialize.** Write `.walkthrough/index.html`, `.walkthrough/playbook.js`, `.walkthrough/playbook.css` and `.walkthrough/serve.js`: the index page lists every catalog entry and links to its slug folder. Then, for each catalog entry, create `.walkthrough/<slug>/` with the same four per-trace files as single-entry mode and write its `data.json`, rich if `_traces.json` carried it, a stub otherwise. Doing this with a small script you write once at `.walkthrough/_build.mjs` keeps it idempotent as the catalog grows.

**Phase 6', launch.** Same `node .walkthrough/serve.js` as single-entry mode, the server serves the playbook at `/` and every slug folder at `/<slug>/`.

**Phase 7', chat handoff.** Print: total entries, how many got rich traces vs stubs, the playbook URL, and instructions to deepen any individual slug by walking that entry point in full. Skip the 5 comprehension questions (they're per-trace, not per-playbook). Skip the decisions checklist.
...
| "Three risks for this node, they're all real." | The risk field is the most-read piece of metadata in the right rail. Three risks means the reader skims. Pick the one that meets the highest bar in 16.11 section 6. If genuinely none, write `"none observed on this path"`. |
...
| "I'll write a fresh viewer for every trace." | When the repo already has a `.walkthrough/<other-slug>/` viewer, copy it; only `data.json` varies. Regenerating drifts. |
...
| "The user said `walk through this PR`, close enough." | A PR is a diff, not an entry point. The code walkthrough traces ONE execution path from ONE entry. If the user means a PR review, that routes through Phase 5 (12.1) or triage (16.2), not here. |

### File map

The per-trace viewer (single-entry mode) lives at `.walkthrough/<slug>/` in the target repo:

```
<repo-root>/.walkthrough/<slug>/
├── index.html         ← the per-trace viewer shell
├── viewer.js          ← copied
├── viewer.css         ← copied
├── serve.js           ← copied
...
In playbook mode the same `.walkthrough/<slug>/` folders exist for every catalog entry, plus a top-level index:

```
<repo-root>/.walkthrough/
├── index.html         ← the playbook index page
├── playbook.js        ← copied
├── playbook.css       ← copied
├── serve.js           ← copied
├── _build.mjs         ← copied from build-playbook.mjs (re-runnable)
├── _catalog.json      ← authored by you in Phase 3'
├── _traces.json       ← optional; authored by you in Phase 4'
...
- It does not modify the traced repo's source code. Every output lives under `.walkthrough/<slug>/`.
...
- It does not navigate cross-process call stacks (RPC, queues across services). External calls are leaves on the trace; if the user wants to follow them, that is a fresh walkthrough rooted in the downstream service.
- It does not replace a plain search for one-off lookups. A search answers `where is X`. A walkthrough answers `what happens when Y fires`.

### One-line summary

**A walkthrough of `<entry-point>`** → **deep depth-first walk to leaves** (controller → service → repo → external + every type/interface/DTO/Zod schema as a `layer: "type"` node) → stop-and-ask only on ambiguity → `.walkthrough/<slug>/{index.html, viewer.js, viewer.css, serve.js, data.json}` → `node serve.js` → browser viewer (light or dark, toggle in header) with every `call_sites` hyperlink resolving to a real node + 5 comprehension questions + decisions checklist. **Playbook mode**: when the user asks for "all endpoints" / "index playbook", author `_catalog.json` + optional `_traces.json`, run `_build.mjs`, and ship a top-level light-mode index with every entry linkable into its own viewer, every per-slug trace inherits the same deep-by-default contract.
