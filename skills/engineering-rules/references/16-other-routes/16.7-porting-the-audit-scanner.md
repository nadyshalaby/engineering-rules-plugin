...
| `ban.empty-catch` | `catch {}` | `except: pass`, bare `except:` | n/a (errors are values) | n/a | `catch (...) {}` |
...
| `ban.non-null` | `x!` | n/a | n/a | `.unwrap()` / `.expect(` (judgment) | n/a |
| `ban.inline-type` | `interface`/`type` in scoped module | dataclass/TypedDict in a router/service module | n/a | n/a | nested class in controller |
