# 3.2 [catalog] Security catalog

## Security Violation Catalog (Canonical)

The canonical catalog of security violations this law scans for. Every entry carries a stable ID that every other surface keys on. Read on demand by: Phase 3 (before touching auth, permission boundaries or any external-input boundary), the Phase 5 security check (12.3) and the challenge (12.8), the law scout's secret rows (9.5), and the codebase audit's (16.3) security category (16.4).

**Canonical direction.** THIS file is canonical, with no always-on distillation; it is one of the three deep, on-demand-only catalogs, with 3.1 and 3.3.
...
| Who | What to load |
|---|---|
| **Phase 3 implementation** | the **severity model + ID scheme above, plus only the domain sections your task's surface actually touches** (auth, authz, injection, ...). A task adding a read-only reporting endpoint does not need the crypto or supply-chain domains. |
| **Phase 5 security check (12.3)** | the whole catalog. It judges every candidate and hunts what nothing else catches, so it needs every domain and every fix direction. |
| **The codebase audit's security category (16.4)** | nothing from here at scan time; it cites these IDs in its own rows, it does not restate them. |
...
Severity in the tables below is the **default**. Context moves it one level: a Minor finding on an internal admin tool may fall further; an Important finding on a path already reachable by an unauthenticated caller rises. the security check (12.3) sets the final severity.

### ID scheme

IDs are stable slugs `sec.<domain>.<slug>`, the same convention as `perf.<domain>.<slug>`. The security check's findings (12.3), the Phase 5 decision table (12.9), and the audit rule catalog (16.4) all cite them. Renaming an ID is a breaking change to every surface that cites it, extend, do not rename.
...
- 3.3, the other no-stub deep catalog, same on-demand-only shape.
- 12.3, the Phase 5 security check this catalog is the reference for.
