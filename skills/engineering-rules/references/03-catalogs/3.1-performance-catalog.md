# 3.1 [catalog] Performance catalog

## Performance Violation Catalog (Canonical)

The canonical catalog of performance violations this law scans for. Every entry carries a stable ID that every other surface keys on. Read on demand by: Phase 3 (before touching data access, loops or hot paths), the Phase 5 performance check (12.4) and the challenge (12.8), the perf scout (9.4), and the codebase audit's (16.3) performance category (16.4).

**Canonical direction.** THIS file is canonical; `SKILL.md` 1.3 is the always-on distillation of it. (For the caps it is the inverse: `SKILL.md` 1.1 is canonical and 2.1 is the deep doctrine.)
...
| Who | What to load |
|---|---|
| **Phase 3 implementation** | the **severity model + ID scheme above, plus only the domain sections your task actually touches** (data access, algorithmic, frontend, ...). A task adding a list endpoint does not need the bundle or logging domains. |
| **Phase 5 performance check (12.4)** | the whole catalog. It judges every staged candidate and hunts what greps miss, so it needs every domain and every fix direction. |
| **Perf scout (9.4)** | nothing from here at scan time; its grep table lives in 9.4. It cites these IDs, it does not read these rows. |
| **Anyone else** | the always-on distillation in `SKILL.md` 1.3, which is already in context. Come here only to cite an ID or read a fix direction. |
...
Severity in the tables below is the **default**. Context moves it one level: a Minor inside a per-request hot loop rises; a Critical inside a one-off migration script may fall. The scout stages findings with the default; the performance check (12.4) sets the final severity.

### ID scheme

IDs are stable slugs `perf.<domain>.<slug>`. Scout grep tables (9.4), staged findings, the Phase 5 decision table (12.9), and the audit rule catalog (16.4) all cite them. Renaming an ID is a breaking change to every surface that greps for it, extend, do not rename.
...
| perf.network.no-keepalive | New connection per outbound request | I | TCP + TLS handshake per call, often longer than the call | client constructed per call; keep-alive disabled | Shared pass/session with keep-alive and pooling |
...
| perf.frontend.blocking-scripts | Render-blocking a build script in the document head | I | First paint waits on every synchronous byte | script tags without defer/async; giant blocking CSS | defer/async scripts; split critical CSS |
...
Performance work obeys the same discipline as everything else, the performance-engineer hat in 5.4: *cheapest-correct beats clever-slow*.
...
- 9.4, the deterministic scout: grep tables keyed to these IDs, staging format, triage rules.
- 16.4, the codebase audit's performance category, which cites this file as canonical.
- 5.4, the performance-engineer hat this catalog operationalizes.
