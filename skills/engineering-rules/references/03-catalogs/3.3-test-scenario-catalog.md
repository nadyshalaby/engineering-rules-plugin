# 3.3 [catalog] Test-scenario catalog

## Test Scenario Catalog (Canonical)

The canonical catalog of test scenarios the testing stage proves breadth against. Every entry carries a stable ID that every other surface keys on. Read on demand by: the Phase 3 testing stage (9.2, before authoring tests for the categories the round touched), the Phase 5 quality-and-plan check (12.7, re-judging every `test.edge-cases` finding), the law scout's semantic tier (9.5), and the codebase audit's (16.3) Testing category (16.4).

**Canonical direction.** THIS file is canonical, and nothing restates it elsewhere. The per-category table in 9.2 names WHICH domains apply to which code category; it does not restate what a domain checks for, why it matters, or how a test for it must be shaped. Those rows live here and only here, the same discipline 3.1 holds for its own violation rows.
...
| Who | What to load |
|---|---|
| **Phase 3 testing stage (9.2)** | the severity model + ID scheme above, plus only the domains the per-category table in 9.2 names for the code category in front of it. A stage testing a pure formatter does not need the authz or concurrency domains. |
| **Phase 5 quality-and-plan check (12.7)** | the whole catalog. It re-judges every `test.edge-cases` finding, so it needs every domain and every test shape. |
| **Law scout (9.5) / the audit's semantic tier (16.6)** | nothing from here at scan time; it cites these IDs in its own finding rows, it does not read these rows. |
...
Severity in the tables below is the **default**. Context moves it one level: an Important gap on an internal admin form may fall to Minor; a Minor encoding gap inside a billing-address field may rise. The semantic tier stages findings with the default; the quality-and-plan check (12.7) sets the final severity.

### ID scheme

IDs are stable slugs `test.<domain>.<slug>`. The testing stage's coverage report, the law scout's semantic-tier findings (9.5), the Phase 5 decision table (12.9), and the audit rule catalog (16.4) all cite them. Renaming an ID is a breaking change to every surface that cites it, extend, do not rename.
...
- 9.2, "What each kind of code owes", the per-category table naming which domains apply to which code category.
- 9.5, the semantic tier's Test coverage row, which judges `test.edge-cases` against this catalog.
- 16.4, the Testing category, whose `test.edge-cases` row cites this file as canonical source.
- 3.1, the sibling catalog this file's shape is modeled on.
