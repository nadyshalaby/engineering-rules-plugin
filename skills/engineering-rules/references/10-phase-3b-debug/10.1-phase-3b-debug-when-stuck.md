# 10.1 [protocol] Phase 3b, Debug when stuck

## Debug When Stuck (Phase 3b)

Triggered when the implement phase fails to make progress after **2 honest attempts** at a fix on the same task. Insert the Phase 3b item into the ledger directly after Phase 3 (5.1) the moment it triggers. Random fixes waste time and create new bugs. **Always find root cause before attempting fixes. Symptom fixes are failure.**
...
When you enter debug mode, **update work-doc frontmatter** (full mode): `status: debugging`. Open a new section in Daily Updates titled `### T<n>, debugging`. In quick mode the hypothesis chain is written in chat under the same heading.
...
Log entry/exit at each, find the layer. Adding temporary logging is allowed only within what the user permitted (6.10, Q4), and every line of it is removed before Phase 4 and proven gone in the cleanup sweep.
...
Write it now, before the fix, and watch it fail for the right reason. Paste the red.
...
The fix stays inside the task's file allowlist. If the root cause lives in a file the plan did not name, that is a scope change: say so, add the file to the backlog with a task, then fix it there.

**The mode is `test-authoring`, with the fix's order: the failing regression test first, then the fix that turns it green** (9.2). The same hands write both halves in the same stage; that is the seam a regression test exists to close.
...
- Tick the Phase 3b ledger item with the root cause in one line, and re-open Phase 3 (5.1, "Loop-back").
