# 14.1 [reference] Anti-patterns, seven worked examples

## Anti-Patterns (Worked Examples)

This doc collects concrete wrong/right diffs for the four working principles. **Read it in Phase 3**, before the first line and whenever an approach feels grand; search here for the matching anti-pattern before committing the diff. **Phase 5 cites it by example number** when flagging over-abstraction, drive-by edits, hidden assumptions, vague goals, suppression rationalizations and scope creep.

Principle text lives in `SKILL.md` 1.6 and is not restated here. The examples below show what the principles look like when they bind, in several languages, so the patterns travel.
...
- The implementation guessed the schema instead of asking; the silent guess shows up later as an incident.
...
- The test surface explodes, instead of pinning one branch, the implementation now owes coverage for a whole new API.
...
The four working principles live in `SKILL.md` 1.6. The mapping below points each example at the principle it most directly breaks.
...
When Phase 3 or Phase 5 cites this doc, cite **the example number and the principle name** so the conversation stays anchored to a concrete pattern, not a vague feeling.
