# 6.10 [reference] Question bank, debug

## Type: `debug`

Phase 1 loads this bank when the user has a mystery to solve rather than a clear ask, intermittent failures, weird behavior with no reliable reproduction. See 6.2 for the canonical 4-section spec.

**SCENARIO**

Use when the user has a mystery to solve rather than a clear ask, weird behavior with no reliable reproduction, intermittent failures, "sometimes X happens and I don't know why." Triggers on patterns like "I'm seeing weird X", "sometimes Y fails", "I can't figure out why", "no error but Z is wrong". The evidence-gathering steps of 10.1 (D1) run during clarify; evidence gathering is part of Phase 1.
...
- If the batch runs past the ~16 target, drop Q3 before either of Q7 and Q8, see 6.5.
...
- Why-this-matters: Sets Phase 3b's evidence toolkit and whether Phase 6 Step D (13.2, class 1) carries a remove-instrumentation cleanup item.
...
- Why-this-matters: Phase 3b inserts a hard checkpoint at the named duration; without it, debug tasks run unbounded past the 3-hypothesis circuit breaker (10.1).
