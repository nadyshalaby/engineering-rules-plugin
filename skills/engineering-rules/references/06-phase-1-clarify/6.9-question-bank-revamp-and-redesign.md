# 6.9 [reference] Question bank, revamp and redesign

## Type: `revamp` or `redesign`

Phase 1 loads this bank for deeper rework where old behavior is being replaced. UI redesign, API redesign, subsystem replacement, framework migration. See 6.2 for the canonical 4-section spec.
...
- Always ask Q6 (Acceptance criteria), gates the acceptance criteria.
...
- Ask Q8 (Known shape) only when the reworked thing sits in a domain listed in 6.12. Skip it for a pure visual refresh with no behaviour change, and say so in the preamble.
...
- If the batch runs past the ~16 target, drop in this order: Q11, then Q8, then Q3. Never drop Q9 or Q10, see 6.5.
- Q5 carries `(Recommended)` on its smallest option as a written default. Re-decide it against what you actually found in the code, per the `(Recommended)` rule in 6.2.
...
- Why-this-matters: Loads the frontend design law (15.1, binding) and the redesign protocol (15.28), whose audit runs before anything is touched. A selects the committed spec as authority; B and C route to 15.5 (Mode B and Mode A); D routes to the read (15.20) and then 15.4.
...
- Why-this-matters: A is preserve mode in the redesign protocol (15.28): it keeps the committed `DESIGN.md` binding, scopes the design-conformance check (12.5) to conformance, and matches the existing site's dials (15.20). B is overhaul mode and C is greenfield; both make authoring or refreshing `docs/design/DESIGN.md` an explicit Phase 2 deliverable, before any component, and require sign-off at the gate.
...
- Why-this-matters: Determines whether a deprecation timeline is emitted and whether the coherence check (12.6) adds a back-compat lens.
