# 6.7 [reference] Question bank, fix

## Type: `fix`

Phase 1 loads this bank when the user is reporting broken behavior with a clear reproduction. See 6.2 for the canonical 4-section spec.
...
- Always ask Q5 (Depth), it gates whether Phase 3 run a refactor task alongside the fix.
...
- Ask Q8 (Prevention) when the bug belongs to a class named in 6.12: a double charge, a lost message, a permission leak, a timezone slip, a retry that ran twice. Skip it when the bug is a one-off mistake with no class behind it, and say so in the preamble.
- If the batch runs past the ~16 target, drop in this order: Q2, then Q8. Never drop Q7 or Q9, see 6.5.
...
- Why-this-matters: Routes the task: stay in `fix` flow (A) vs run the evidence-gathering of 10.1 first (B/C). No reliable repro means no honest RED test.
...
- Why-this-matters: Decides whether 11.4 runs and whether the coherence check's (12.6) seam sweep needs a wider consumer grep.
...
- Why-this-matters: Determines whether Phase 3 runs one implementation task or also a refactor task, and whether a Retrospective follow-up is filed.
...
- Why-this-matters: Determines whether the fix owes a watched RED that reproduces this exact bug, written before the fix (9.2), and proven again at Phase 4 by reverting the fix and requiring a red that names it (11.2). Recommend A unconditionally unless the user prompt contains `no test`, `quick fix only`, or `can't test`.
...
- Why-this-matters: Sits beside Q5 rather than replacing it. Q5 asks how deep to go into THIS bug's cause; this asks whether to add the mechanism that makes the class impossible, which is a different and usually larger decision. A adds a task to the Sprint Backlog; B files a Retrospective follow-up naming the class. Source the mechanism from 6.12 and never name one without the failure it prevents, per the honesty rule in 6.2.
...
- Why-this-matters: The half-fix this bank kept shipping. A code fix leaves a repair task unwritten, and nothing downstream notices, because the tests pass against the new behaviour. A adds a data-repair task to the Sprint Backlog with its own Evidence Ledger row (count before, count after). Never state a number of affected records that you did not count, per the honesty rule in 6.2.
