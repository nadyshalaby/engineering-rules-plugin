# 6.12 [reference] Domain mechanisms, twelve domains
...
1. **Name the mechanism, name the failure.** "Every write carries an idempotency key, so a retry after a timeout cannot charge twice" is a fact the user can weigh. "This is the standard approach" is not. The full rule is the honesty rule in 6.2, and it binds every question that cites this file.
...
Some requests touch none of these. A colour change, a copy edit, a log line. Then there is no proven flow to offer and the question is cut, per the composition rules in 6.5. An invented mechanism is worse than a missing question.
