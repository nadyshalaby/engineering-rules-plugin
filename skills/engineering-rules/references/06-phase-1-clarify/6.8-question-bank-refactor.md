# 6.8 [reference] Question bank, refactor

## Type: `refactor`

Phase 1 loads this bank when behavior should NOT change but structure should, moving, renaming, extracting, deduplicating, restoring layer boundaries. See 6.2 for the canonical 4-section spec.
...
- Always ask Q7. A rework with no named payoff is the easiest plan to grow without anyone deciding it should, and option D is the only place the user is offered "this may not be worth doing".
...
- Ask Q8 (Target shape) only when the restructured code sits in a domain listed in 6.12. Skip it for a pure rename or a file move with no structural target, and say so in the preamble.
- If the batch runs past the ~16 target, drop in this order: Q6, then Q8, then Q3. Never drop Q7 or Q9, see 6.5.
...
- Why-this-matters: Frames the plan narrative and which Phase 5 check carries the most weight (12.7 quality and layering, or 12.4 performance).
...
- Why-this-matters: Decides whether 11.4 runs and how many stages Phase 3 needs.
...
- Why-this-matters: A refactor with no named target shape becomes a matter of taste, and taste is what makes the Phase 5 quality review unarguable in both directions. Source the mechanisms from 6.12 and never state one without the failure it prevents, per the honesty rule in 6.2. The accepted shape becomes the Approach section and the standard the quality-and-plan check (12.7) judges the diff against.
...
- Why-this-matters: A rename is measured by its callers, not by its own diff, and callers outside this repository are invisible to every tool here. Sits beside Q5 rather than replacing it: Q5 asks how the change is released, this asks who has to change with it. Answer B makes deprecation shims a Sprint Backlog task and adds a back-compat lens to the coherence check (12.6); D inserts a consumer sweep before the plan is drafted. Never state a caller count you did not grep for, per the honesty rule in 6.2.
