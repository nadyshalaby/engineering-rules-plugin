# 6.6 [reference] Question bank, feature

## Type: `feature`

Phase 1 loads this bank when the user is adding new behavior the system doesn't currently have. See 6.2 for the canonical 4-section spec.
...
- Always ask Q1 (Goal shape), it locks the North-Star Goal, confirmed or corrected in the user's own words.
...
- Always ask Q7 (Acceptance criteria), it gates the acceptance criteria.
...
- Ask Q9 (Proven shape) only when the request touches a domain listed in 6.12. When nothing matches, skip Q9 and say so in the preamble. An invented mechanism is worse than a missing question.
...
- If the batch runs past the ~16 target, drop in this order: Q12, then Q9, then Q3. Never drop Q10 or Q11, see 6.5.
- Q4, Q5 and Q6 carry `(Recommended)` on their smallest option as a written default. Re-decide it against the Q9 answer before sending, per the `(Recommended)` rule in 6.2.
...
- Why-this-matters: Drives the file allowlist (placed per 2.2) and whether 11.4 runs.
...
- Why-this-matters: Determines whether a migration task enters the Sprint Backlog and whether the expand-then-contract pattern applies. B and C make the security check's (12.3) migration lens load-bearing.
...
- Why-this-matters: Decides whether route registration and API docs are touched, and makes the security check's (12.3) auth/permission lens load-bearing in Phase 5.
...
- Why-this-matters: Triggers (or skips) the frontend design law (15.1) and makes the design-conformance check (12.5) a standing reviewer on the diff for any answer other than A. Any answer other than A also declares the design read and the dials (15.20) in the preamble and adds the design scout (15.29) to every stage end that writes a UI file.
...
- Why-this-matters: A makes the committed spec's tokens binding in Phase 3. B routes to 15.5 Mode A. C routes to 15.20, then 15.4 and 15.6, and makes `docs/design/DESIGN.md` the first deliverable, authored BEFORE any component (15.19). A brief that names an official design system routes to 15.21 whatever the answer.
...
- Why-this-matters: This is the business rule no code review catches, because the code runs correctly either way. Draw the candidate rules from the "Correctness rules that bite here" list for the matched domain in 6.12: rounding and currency, which timezone a day means, what order events apply in, who may see whose records, what has to be auditable, what happens on a partial failure. The answer becomes an Acceptance Criteria bullet AND a named test case. If two rules genuinely bite, ask the sharper one here and state the other in the preamble for a one-line confirmation.
- Recommend: Lead with whichever answer the mechanism in 6.12 supports, not with whichever is easier to build. Where both answers are defensible, say that in the option's own text rather than projecting certainty.
...
- Why-this-matters: This question exists to make the right call visible in BOTH directions. A real answer of a few hundred records makes the simple design correct and building for millions the waste; a real answer in the millions makes the simple design a defect. Never state a volume figure the user did not give and you did not count, per the honesty rule in 6.2. The answer sets whether pagination, indexing and background work enter the Sprint Backlog at all.
