# 6.2 [contract] The question contract

## Wizard Contract

Phase 1 builds **one batched questionnaire** drawn from the universal preamble (6.4) plus the bank for the matched task type (6.6 to 6.11). **Drop questions whose answer is already evident** from the user's prompt or from context you have already read (the code, the file system, `CLAUDE.md`). Add task-specific questions if a bank misses something obvious, and write them to this same contract.

### Delivery format, wizard only (mandatory, every phase)

**Every question put to the user, in every phase, is delivered through the wizard tool (`AskUserQuestion` on Claude Code).** Not just Phase 1 clarify: the Phase 2 gate and any re-gate after a plan change, the Phase 5 fix-approval batches, the Phase 6 landing menu, the walkthrough offer, a Phase 3b "which branch do you want me to chase" fork, a pre-existing-issues offer in the cleanup sweep. If you are asking the user to decide something, it goes through the wizard. Plain numbered markdown lists in chat are forbidden, the wizard renders structured options the user can click, which is faster, less error-prone, and easier to answer on the move.
...
**A question the user cannot answer without knowing how this process works internally is a broken question.** This is the single most common defect in a bank. The user has not read the work-doc, does not know what a task ID is, and should never have to.
...
- **Phase references.** "Phase 2", "the gate", "Phase 4 cross-package verification", "in stage 2".
- **Internal artifact names.** Acceptance criteria as a term of art, work-doc, Sprint Backlog, Daily Updates, goal anchor, stage, scout, ship gate, the quality-and-plan check, decision table, phase ledger, Evidence Ledger.
...
If the answer genuinely changes something internal, say the *effect* in plain words instead. "This decides whether I write a database migration" is fine. "This drives the migration lens in the review" is not.
...
In a bank file it is written as an indented `What happens:` line directly under its option, and it becomes the wizard option's `description` field at run time:
...
An option with no `What happens:` line, or one that restates its label, is a bank defect.
...
The mechanisms, and the failure each one prevents, are collected in 6.12. A recommendation you cannot trace to a mechanism there, or to something you read in the user's own code, is one you do not make.

**Say where it was verified.** A recommendation that rests on how a library, a framework,
an API, a tool or a platform behaves carries the place that behaviour was verified this
session: `verified in your repo (<file:line>)`, `verified in <package>@<version> source`,
`verified in docs fetched now (<url>)`, or, honestly, `an idea I have not verified; worth a
spike before we lean on it`. What the model remembers is a hypothesis until one of those
stands behind it (1.5, "Memory is a hypothesis"), and a hypothesis is never worded as a
fact. The user can only weigh what they can check.
...
#### Recommendations beyond the questions

A senior engineer brings options the user did not ask for, and this phase is where they
are cheapest. A `Suggestions:` block follows the "What I heard" preamble whenever you have
one: a tool or helper already in the repo that does part of the work (named, `file:line`), a
simpler path that reaches the same outcome with fewer moving parts, a mechanism from 6.12
the request could borrow, an experiment or a spike that would settle an open risk, a way
to get the value sooner. Each suggestion carries its evidence tier (the line above) and
its gain in things the user can count: screens not built, tables not changed, places not
edited, a failure that cannot happen any more. A suggestion that forks a decision goes into
the batch as an option with `(Recommended)` decided by the same rules as any other; one
that does not fork anything is stated once and not pressed. Nothing in the block is
dressed up as settled when it is a hunch, and nothing in it comes from memory alone.
...
B. I'll write one sentence now in chat saying what done looks like
...
Read every question as if you had never seen this codebase or this process. If any of these is true, rewrite it:
...
4. It asks about your process rather than the user's product.
...
9. Its `traces-to` line names no deliverable or requirement in the anchor; the question is decoration.
10. Its `cost-of-guessing` line is empty, or says "minor" for a fork that would cost a stage, a security boundary or a customer's money.
11. It states how a library, a framework, a tool or a platform behaves, and nothing verified that this session (1.5, "Memory is a hypothesis").
12. The batch was composed before the coverage map (6.15) was walked, so an aspect nobody thought of has no mark.

---

### Tool constraints to design around

- **1-4 questions per call.** If your batch is larger, send **multiple back-to-back `AskUserQuestion` calls in the same turn**, firing the next call as soon as the previous batch is answered, with no chat narration in between unless something needs clarifying. Around sixteen questions across all batches is the soft target (5.2): not a ceiling when a part of the anchor is still unpinned, not a quota to fill. If you are far past it, the scope is too broad; narrow first.
...
- Order: the intent restatement (6.4, Q0) first, alone or with the guardrails question; then scope-shaping questions (anything whose answer changes which other questions matter), then data model, then UX, then logistics (worktree, tests, output). Among questions of the same shape, the one whose `cost-of-guessing` is highest goes first: a wrong guess about money or a security boundary is asked before a wrong guess about a label.
...
Section 14.1 carries seven worked anti-pattern examples. Phase 1 does not read it (you are not writing code yet); Phase 3 does. Keep it in mind when drafting a question that touches a known anti-pattern (over-abstraction, scope creep, a suppression rationalized as "complex").

---

### Purpose

Every wizard bank (the universal preamble plus the six `Type:` task-type banks) MUST conform to the 4-section structure defined here. This is the canonical specification; a bank that drifts from it is a defect. RFC 2119 keywords (MUST / SHOULD / MAY) apply throughout.
...
3. **QUESTIONS**, the candidate question pool (4-8 per bank). Each question MUST conform to the Question structure (following subsection). Questions whose answer is already evident from context MUST be dropped at composition time, not asked.
   ...
   *Mini-example:* "Q1. Scope. text: `Single file or cross-module?` header: `Scope`. options: A `Single file (Recommended)` / B `Cross-module` / C `Cross-project`. why-this-matters: determines whether the worktree is created and whether Phase 4 cross-package verification (11.4) runs."

4. **EXIT CRITERIA**, the binary condition under which the wizard is "done enough" to proceed to the next phase. MUST be checkable, not aspirational. You use this to decide whether to loop back for another batch or move on.
...
- **traces-to**, one line naming the deliverable or requirement in the anchor (5.2) this question settles. **Model-facing only.** A question that traces to nothing is cut; the coverage map (6.15) reads this line to mark the aspect `asked`.
- **cost-of-guessing**, one line naming what a wrong guess would cost if the question were not asked: a stage rebuilt, a migration reversed, a security boundary crossed, money moved twice, a customer misled. **Model-facing only.** It decides the order inside a batch and whether the question survives the drop order in 6.5; a question whose wrong guess costs nothing is cut.
- **why-this-matters**, one line stating what the answer changes downstream (which task-type branch is taken, which plan section is generated, which review check becomes load-bearing, which verification step runs). **Model-facing only, never rendered.** Internal vocabulary is correct here. If the answer changes nothing downstream, the question MUST be cut.
...
Generic "use judgment" or "ask what feels relevant" language is forbidden. You, and a much weaker model executing this skill, MUST be able to mechanically apply the COMPOSITION rules without inferring intent.
