...
| **The agent tool** | The runtime's subagent tool: `Agent` on Claude Code, with `subagent_type: "fork"` for a reader and a fresh type for a reviewer. This plugin ships the reviewer, mechanic and builder shapes as `engineering-rules:law-reviewer`, `engineering-rules:law-mechanic` and `engineering-rules:law-builder` (5.5). Section 1.8 is this skill's standing request for it. |
